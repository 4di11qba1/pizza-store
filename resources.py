...
class AdminLogin(Resource):
    def post(self):
        ...
        if 'admin' in session:
            return jsonify({"message": "Already logged in", "admin": session['admin']})

        # Fetch admin from the database
        admin = Admin.objects(username=data['username']).first()

        if admin and admin.password == data['password']:  # Ideally, hash comparison should be used
            session['admin'] = admin.username  # Store admin in session
            return jsonify({"message": "Login successful", "admin": admin.to_json()})
...
class AdminLogout(Resource):
    def get(self):
        ...
        if 'admin' in session:
            session.pop('admin')  # Remove admin from session
...
class AddRider(Resource):
    def post(self):
        ...
        if not rider_name:
            return jsonify({"message": "Rider name is required!"})
